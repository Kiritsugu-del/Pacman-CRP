...
class Moved(Predicate):
    # (agent, x, y, dx, dy)
    def __init__(self, agent: Term, x: Term, y: Term, dx: Term, dy: Term):
        super().__init__(agent, x, y, dx, dy)
        self.agent, self.x, self.y, self.dx, self.dy = self.args

class NextLikely(LocationBasedPredicate):
    # (x, y)
    def __init__(self, x: Term, y: Term):
        super().__init__(x, y)